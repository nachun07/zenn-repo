...
ChatGPTは便利ですが、有料版は月額20ドル必要です。また、無料版でも機能制限があります。加えて、ローカル環境で動かしたい、プライベートなデータを扱いたい、インターネット接続がない環境で使いたい、という場面もあります。
...
Ollamaのインストールは非常にシンプルです。Homebrewを使う方法が最も簡単です。
...
インストールが成功したか確認するために、Ollamaのバージョンを表示します。

```bash
ollama --version
```

バージョン情報が表示されれば、インストール成功です。例えば、以下のような出力が得られます。
...
Ollamaはバックグラウンドサービスとして動作します。初回は以下のコマンドで起動してください。

```bash
ollama serve
```

このコマンドを実行すると、ターミナルに以下のようなメッセージが表示されます。

```
time=2024-01-15T10:30:00.123Z level=INFO msg="Listening on 127.0.0.1:11434"
```

これはOllamaがポート11434で待ち受けているという意味です。このターミナルは起動したままにしておいてください。

実際に使う際は、新しいターミナルウィンドウを開いて、そこで次のステップを進めます。

## モデル実行方法

Ollamaが起動した状態で、実際にLLMモデルを実行します。
...
ここでテキストを入力できます。例えば、以下を試してみてください。

```
>>> 日本語で「こんにちは」と言った場合、適切な返答は何ですか。
```

モデルが考えて、以下のような回答が返ってきます。

```
「こんにちは」と言った場合、適切な返答は「こんにちは」または「こんにちは、お疲れ様です」などです。

状況や相手との関係によって、返答は異なります：

- 友人や同僚への気軽な返答：「こんにちは」「よろしく」
- 目上の人や初対面の人への返答：「こんにちは、○○です」「こんにちは、初めまして」
- 敬意をこめた返答：「こんにちは、いつもお世話になっております」

基本的には、相手の挨拶に同じ形で返すのが最も一般的で、安全です。
```

複数回の会話も可能です。そのまま続けて入力できます。

```
>>> そうなんですね。では「さようなら」の場合は。
```
...
Ollamaはコマンドラインでも利用できます。スクリプトから呼び出す際に便利です。
...
APIを使う場合、Ollamaは常に起動していないといけません。ターミナルで以下を実行してください。

```bash
ollama serve
```

このターミナルウィンドウは常に開いたままにします。
...
class OllamaChat:
    ...
    def send(self, user_message):
        ...
        """
        # 会話履歴にユーザーメッセージを追加
        self.history.append(f"ユーザー: {user_message}")
        ...
        if response.status_code == 200:
            ...
            # 会話履歴にアシスタントメッセージを追加
            self.history.append(f"アシスタント: {assistant_message}")
...
5. **日本語対応の差**
   モデルによって日本語対応の品質が異なります。Llama3は日本語対応がやや弱いため、Qwenなどのほうが日本語出力には向いている傾向があります。
...
実際にMacBook Pro（M1チップ）で試してみました。

Llama3.1は4.7GBのダウンロードで、ダウンロード自体は5分程度で終わりました。セットアップは簡単で、インストール後すぐに使える状態になります。

対話モード、CLIコマンド、APIの3つの方法すべてが問題なく動作しました。特にPythonのAPIでは、Ollamaをバックグラウンドで実行しておくだけで、簡単にプログラムに組み込めました。

日本語の対応は、Llama3でも基本的な日本語は理解していますが、複雑な文脈や専門用語は Qwenのほうが得意という印象です。
