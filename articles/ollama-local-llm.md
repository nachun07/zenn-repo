...
公式サイトのトップページに「Download」ボタンがあります。クリックします。

macOSバージョン選択画面が表示されます。

- **Apple Silicon（M1/M2/M3チップ搭載Mac）**の場合：「Download for Mac - Apple Silicon」をクリック
- **Intel Mac**の場合：「Download for Mac - Intel」をクリック

自分のMacのチップを確認する方法：
- メニューバーの左上、Appleロゴをクリック
- 「このMacについて」を選択
- 「チップ」の項目を確認

#### ステップ3：インストール

ダウンロードが完了すると、`Ollama-x.x.x.dmg`というファイルがダウンロードフォルダに保存されます。

このファイルをダブルクリックするとインストーラーが開きます。

```
Ollamaというアプリケーションを Applications フォルダにコピーしてもよろしいですか？
```

このダイアログが表示されたら「コピー」をクリックします。
...
以下のような応答が返ってきたら、Ollamaが正常に動作しています。

```json
{"models":[]}
```

`{"models":[]}` と表示されるのは、まだモデルをダウンロードしていない状態です。これは正常です。
